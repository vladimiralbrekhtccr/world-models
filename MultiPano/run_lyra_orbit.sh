#!/bin/bash
# Single Lyra-2 run with a 360° in-place yaw sweep trajectory.
set -e
...
STAGE="$LYRA/inputs/wm_pano2_orbit"
rm -rf "$STAGE" && mkdir -p "$STAGE"
cp "$WM/lyra_in_orbit/"*.png "$WM/lyra_in_orbit/"*.npz "$WM/lyra_in_orbit/"*.json "$STAGE/"

OUTDIR="$LYRA/outputs/pano2_orbit"
rm -rf "$OUTDIR" && mkdir -p "$OUTDIR"

echo "=== Step 1: video diffusion ==="
CUDA_VISIBLE_DEVICES=0 PYTHONPATH=. python -m lyra_2._src.inference.lyra2_custom_traj_inference \
  ...
  --output_path "$OUTDIR" --num_frames 161 --num_samples 1 \
